...
from flask import Blueprint, render_template, redirect, abort, url_for, request, flash, send_from_directory
from werkzeug.utils import secure_filename
from flask_login import current_user, login_user, logout_user

import app.utils.utilites as utilites
from app import db
from app.infrastructure.models import Posts, Category, Users, Images
...
def posts(cat_name="", alias=""):
    ...
    if alias == "":
        articles = Posts.query.filter_by(cat_id=category.id)
        return render_template("/articles.html", ARTICLES=articles, CATEGORY=category, CATEGORYES=categoryes)

    article = Posts.query.filter_by(alias=alias).first()
    if article is None:
